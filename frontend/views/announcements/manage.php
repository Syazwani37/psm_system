...
<div class="page-container">
    <div class="d-flex align-items-center mb-5">
        <a href="<?php
require_once dirname(__FILE__, 3) . '/backend/config/database.php'; echo BASE_URL; ?>/frontend/views/dashboard/professional.php" class="btn btn-outline-secondary me-3 rounded-circle shadow-sm" style="width: 45px; height: 45px; display: flex; align-items: center; justify-content: center;" title="Back to Dashboard">
    ...
    <div class="form-card">
        ...
        <form action="<?php
require_once dirname(__FILE__, 3) . '/backend/config/database.php'; echo BASE_URL; ?>/backend/api/announcements/create.php" method="POST">
            <div class="mb-3">
...
require_once dirname(__FILE__, 3) . '/backend/config/database.php'; echo $typeClass; ?> rounded-pill" style="font-size: 0.7rem;"><?php
require_once dirname(__FILE__, 3) . '/backend/config/database.php'; echo strtoupper($row['type']); ?></span>
                            <span class="text-muted small"><?php
require_once dirname(__FILE__, 3) . '/backend/config/database.php'; echo date('M d, Y h:i A', strtotime($row['created_at'])); ?></span>
                        </div>
                        <h6 class="mb-1 fw-bold"><?php
require_once dirname(__FILE__, 3) . '/backend/config/database.php'; echo escape($row['title']); ?></h6>
                        <p class="text-secondary small mb-0"><?php
require_once dirname(__FILE__, 3) . '/backend/config/database.php'; echo escape($row['message']); ?></p>
                    </div>
                    <form action="<?php
require_once dirname(__FILE__, 3) . '/backend/config/database.php'; echo BASE_URL; ?>/backend/api/announcements/delete.php" method="POST" onsubmit="return confirm('Are you sure you want to delete this?');">
                        <input type="hidden" name="announcement_id" value="<?php
require_once dirname(__FILE__, 3) . '/backend/config/database.php'; echo $row['id']; ?>">
