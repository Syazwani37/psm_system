...
require_once $_SERVER['DOCUMENT_ROOT'] . '/backend/config/database.php';
            while ($ann = $ann_result->fetch_assoc()) {
                ...
                ?>
                <div class="alert <?php
require_once $_SERVER['DOCUMENT_ROOT'] . '/backend/config/database.php'; echo $alertClass; ?> border-0 rounded-3 shadow-sm mb-2 d-flex align-items-start gap-3" role="alert">
                    <i class="fas <?php
require_once $_SERVER['DOCUMENT_ROOT'] . '/backend/config/database.php'; echo $icon; ?> mt-1 fa-lg"></i>
                    <div>
                        <h6 class="alert-heading fw-bold mb-1"><?php
require_once $_SERVER['DOCUMENT_ROOT'] . '/backend/config/database.php'; echo escape($ann['title']); ?></h6>
                        <p class="mb-0 small opacity-75"><?php
require_once $_SERVER['DOCUMENT_ROOT'] . '/backend/config/database.php'; echo escape($ann['message']); ?></p>
                        <small class="text-muted" style="font-size: 0.7em;"><?php
require_once $_SERVER['DOCUMENT_ROOT'] . '/backend/config/database.php'; echo date('M d, h:i A', strtotime($ann['created_at'])); ?></small>
                    </div>
                </div>
                <?php
require_once $_SERVER['DOCUMENT_ROOT'] . '/backend/config/database.php';
