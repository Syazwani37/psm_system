...
<div class="page-container">
    <div class="text-center mb-5">
        <a href="<?php
require_once dirname(__FILE__, 3) . '/backend/config/database.php'; echo BASE_URL; ?>/frontend/views/patient_management/index.php" class="btn btn-outline-secondary btn-sm mb-3">
            <i class="fas fa-arrow-left me-2"></i> Back to Patient List
        </a>
        <h1 class="mb-2" style="font-family: 'Playfair Display', serif; color: #00695C;">
            Log History: <?php
require_once dirname(__FILE__, 3) . '/backend/config/database.php'; echo escape($patientName); ?>
        </h1>
...
require_once dirname(__FILE__, 3) . '/backend/config/database.php'; echo date('M d, Y', strtotime($row['created_at'])); ?></div>
                                    <div class="small text-muted"><?php
require_once dirname(__FILE__, 3) . '/backend/config/database.php'; echo date('h:i A', strtotime($row['created_at'])); ?></div>
                                </td>
                                <td>Week <?php
require_once dirname(__FILE__, 3) . '/backend/config/database.php'; echo escape($row['week_postpartum']); ?></td>
                                <td>
                                    <div><i class="fas fa-thermometer-half text-secondary me-1"></i> <?php
require_once dirname(__FILE__, 3) . '/backend/config/database.php'; echo escape($row['temperature']); ?>°C</div>
                                    <div><i class="fas fa-sad-tear text-secondary me-1"></i> Pain: <?php
require_once dirname(__FILE__, 3) . '/backend/config/database.php'; echo escape($row['pain_level']); ?>/10</div>
                                </td>
                                <td><?php
require_once dirname(__FILE__, 3) . '/backend/config/database.php'; echo $statusBadge; ?></td>
                            </tr>
                            <?php
require_once dirname(__FILE__, 3) . '/backend/config/database.php';
                        }
