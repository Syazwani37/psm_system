...
require_once dirname(__DIR__, 3) . '/backend/config/database.php'; displayFlashMessage(); ?>
    ...
    <?php
require_once dirname(__DIR__, 3) . '/backend/config/database.php'; if ($_SESSION['role'] === 'professional'): ?>
...
require_once dirname(__DIR__, 3) . '/backend/config/database.php'; echo $iconClass; ?>"><i class="fas <?php
require_once dirname(__DIR__, 3) . '/backend/config/database.php'; echo $icon; ?>"></i></div>
                    <div class="resource-title"><?php
require_once dirname(__DIR__, 3) . '/backend/config/database.php'; echo escape($row['title']); ?></div>
                    <div class="resource-desc"><?php
require_once dirname(__DIR__, 3) . '/backend/config/database.php'; echo escape($row['description']); ?></div>
                    
                    <a href="<?php
require_once dirname(__DIR__, 3) . '/backend/config/database.php'; echo BASE_URL . $row['file_path']; ?>" class="download-btn mb-2" download>
                        <i class="fas fa-download"></i> Download
                    </a>

                    <!-- Delete Option (Professional Only) -->
                    <?php
require_once dirname(__DIR__, 3) . '/backend/config/database.php'; if (isset($_SESSION['role']) && $_SESSION['role'] === 'professional'): ?>
                    <form action="<?php
require_once dirname(__DIR__, 3) . '/backend/config/database.php'; echo BASE_URL; ?>/backend/api/delete_resource.php" method="POST" class="w-100" onsubmit="return confirm('Are you sure you want to delete this resource?');">
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content" style="border-radius: 20px; border: none;">
            ...
            <div class="modal-body p-4">
                <form action="<?php
require_once dirname(__DIR__, 3) . '/backend/config/database.php'; echo BASE_URL; ?>/backend/api/upload_resource.php" method="POST" enctype="multipart/form-data">
                    <div class="mb-3">
