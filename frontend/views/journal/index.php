...
require_once dirname(__FILE__, 3) . '/backend/config/database.php'; displayFlashMessage(); ?>
    ...
    <!-- Analytics Summary -->
    <?php
require_once dirname(__FILE__, 3) . '/backend/config/database.php'; if($dominant_mood): ?>
    <div class="summary-banner">
        <div>
            ...
            <div style="font-size: 2.5rem; font-weight: 700; line-height: 1.2;">
                <?php
require_once dirname(__FILE__, 3) . '/backend/config/database.php'; echo ucfirst($dominant_mood); ?>
            </div>
...
require_once dirname(__FILE__, 3) . '/backend/config/database.php'; endif; ?>
    ...
    <?php
require_once dirname(__FILE__, 3) . '/backend/config/database.php'; if(empty($entries)): ?>
...
require_once dirname(__FILE__, 3) . '/backend/config/database.php'; else: ?>
        <div class="d-flex flex-column gap-3">
            <?php
require_once dirname(__FILE__, 3) . '/backend/config/database.php'; foreach($entries as $e): ?>
                <?php
require_once dirname(__FILE__, 3) . '/backend/config/database.php'; 
                $mood = $e['mood'] ?? 'happy';
                $icons = [
...
require_once dirname(__FILE__, 3) . '/backend/config/database.php'; echo escape($mood); ?>" style="border-radius: 16px;">
                    <div class="card-body p-3">
                        <div class="d-flex justify-content-between align-items-start mb-2">
                            <small class="text-muted fw-bold">
                                <?php
require_once dirname(__FILE__, 3) . '/backend/config/database.php'; echo formatDateTime($e['created_at']); ?>
                            </small>
                            <i class="fas <?php
require_once dirname(__FILE__, 3) . '/backend/config/database.php'; echo $iconClass; ?> fa-lg" title="<?php
require_once dirname(__FILE__, 3) . '/backend/config/database.php'; echo ucfirst($mood); ?>"></i>
                        </div>
                        <p class="card-text mb-0" style="white-space: pre-wrap; color: #555;"><?php
require_once dirname(__FILE__, 3) . '/backend/config/database.php'; echo escape($e['entry_text']); ?></p>
                    </div>
                </div>
            <?php
